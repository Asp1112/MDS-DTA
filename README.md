...
├── models/                   # model definition (models/MDS_dta.py), checkpoints, ablation variants
├── ablation/                 # ablation results (six variants, per-fold summaries)
...
Run the full six-fold cross-validation with
[`run_sixfold_cv.py`](run_sixfold_cv.py):
...
Processed tensors and deterministic six-fold splits are generated by the
scripts in [`data_prep/`](data_prep/) (see
[`data_prep/README.md`](data_prep/README.md)).

### Supplementary experiments

Cold-start, randomization and reduced-data experiments are under
[`experiments/`](experiments/) with their prepared datasets, splits and
training scripts (see [`experiments/README.md`](experiments/README.md)).

### Ablation

The six ablation variants are in [`models/ablation/`](models/ablation/); the
reported six-fold results are in [`ablation/ablation_results.csv`](ablation/ablation_results.csv)
and per-fold summaries under [`ablation/results/`](ablation/results/).
