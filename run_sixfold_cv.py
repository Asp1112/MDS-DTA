...
example the currently running MDS_dta experiment) to finish
before starting.
...
Examples:
  ...
  python run_sixfold_cv.py --model MDS_dta --all-folds
  python run_sixfold_cv.py --model MDS_dta --start-now
  python run_sixfold_cv.py --model MDS_dta --dry-run
"""
...
ROOT = "/root/mds"
PYTHON = "/root/miniconda3/envs/mds/bin/python"
...
def dataset_done(dataset, fold, model):
    pattern = os.path.join(ROOT, "results", "cv", dataset,
                           model, "*_fold%d_*" % fold, "test_metrics.json")
...
def main():
    ...
    parser.add_argument("--model", required=True,
                        help="Model module name under models/, e.g. "
                             "MDS_dta. Fill in the model you want "
                             "to evaluate.")
            ...
            for dataset in args.datasets:
                for fold in args.folds:
                    if dataset_done(dataset, fold, args.model):
                        print("  [skip] %s fold %d (already completed for %s)"
                              % (dataset, fold, args.model))
                    else:
