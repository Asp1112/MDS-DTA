"""Base CombinedDTA + a single control point: bond-type (edge_attr) usage.

This module is byte-for-byte identical to ``combined_dta.py`` except for ONE
control point: the graph encoder additionally reads ``data.edge_attr`` and
injects bond-type embeddings into the initial node features before message
passing.  Everything else (encoders, fusion, head, hyper-parameters) is
unchanged, so comparing ``combined_dta`` vs ``combined_dta_edge`` isolates
the effect of consuming edge attributes.

The edge-attr call is a no-op when ``edge_attr`` is absent from the data
(e.g. the ``*_sixfold_all_noedge.pt`` versions), so this module also runs on
legacy data.

Bond-type formats are handled automatically:
  * 1-D integer labels (davis/kiba: 0..3) -> used directly;
  * one-hot rows (bindingdb: 5-dim) -> argmax.
"""
...
class GraphEncoder(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_steps, proj_dim, dropout,
        ...
        # --- control point: bond-type (edge_attr) embedding ---
        self.bond_emb = nn.Embedding(num_bond_types + 1, hidden_dim)
        self.bond_norm = GraphNorm(hidden_dim)
    ...
    def forward(self, data):
        ...
        # --- control point: consume edge_attr if present (no-op otherwise) ---
        edge_attr = getattr(data, "edge_attr", None)
        if edge_attr is not None:
            ea = edge_attr
            if ea.dim() > 1 and ea.size(-1) > 1:
                ea = ea.argmax(dim=-1).long()
            else:
                ea = ea.long()
            ea = ea.clamp(0, self.bond_emb.num_embeddings - 1)
            bond = self.bond_emb(ea)  # [E, hidden]
            agg = torch.zeros_like(x0)
            agg = agg.index_add(0, data.edge_index[1], bond)
            x0 = x0 + self.bond_norm(agg, data.batch)
...
class CombinedDTAEdge(nn.Module):
    """Base CombinedDTA + the single edge_attr (bond-type) control point."""
    ...
    def __init__(self,
        ...
        self.drug_encoder = GraphEncoder(in_dim=drug_atom_feat_dim, hidden_dim=graph_hidden,
                                         n_steps=graph_steps, proj_dim=common_dim,
                                         dropout=dropout, num_bond_types=num_bond_types)

        self.fusion = CrossAttentionFusion(
            embed_dim=common_dim, heads=heads, dropout=dropout)
