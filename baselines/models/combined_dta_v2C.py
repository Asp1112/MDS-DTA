"""CombinedDTA V2-C (ablation: token-level cross-attention + mean/max pool).

Built from the original combined_dta.py (no LSTM inter-layer dropout).
Same token-level masked bidirectional cross-attention fusion as V2, but the
global summary after fusion uses the original masked mean+max pooling
combination instead of the learned-query attention pooling. This isolates
the effect of the final pooling choice.
"""
...
def mean_max_pool(tokens, mask):
    ...
    seq_masked = tokens.masked_fill(~mask.unsqueeze(-1), -1e9)
...
class CombinedDTAV2C(nn.Module):
    def __init__(self,
                 protein_vocab=27,
