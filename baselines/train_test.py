"""train_test.py - clean training-side harness for the CombinedDTA family.

Plain MSE loss only.  The earlier margin / floor / label-noise terms are
removed: the Davis experiments showed that the pairwise margin destroys
calibration from epoch 1 (69.6% of labels are tied at the 5.0 floor, so the
ranking signal fights the regression signal), and the one-sided floor loss
removes the floor labels from regression entirely (zero gradient below the
floor), which inflated validation MSE on the floor mass.

Everything else matches the formal baseline train.py: fixed 4/1/1 six-part
split, validation-based selection, top-3 checkpoint averaging, grad clip 5,
AMP, seed 42.  The two training knobs left under test:

  * AdamW with configurable weight decay (default 1e-4);
  * scheduler: plateau (default, the recipe behind the healthy baseline) or
    cosine with a short period (T_max 300, eta_min 1e-5) + warmup, to be used
    only as a single-factor comparison.

Examples:
  python train_test.py --model combined_dta_control
  python train_test.py --model combined_dta_aug --scheduler cosine
  python train_test.py --model combined_dta_token
  python train_test.py --model combined_dta_enhanced   # ESM downloads on first forward
"""
...
class IndexedSubset(torch.utils.data.Dataset):
    ...
    Needed by the cached-ESM path: the precomputed ESM features are aligned
    with the full dataset order, so each batch must know the global index of
    every sample (stored as ``data.esm_idx``).
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Train CombinedDTA-family models (plain MSE, clean recipe).")
    parser.add_argument("--dataset", default="davis")
    parser.add_argument("--model", default="combined_dta_control")
